...
function formatTimeOfDayTitle(date) {
  ...
  return `${months[date.getMonth()]} ${date.getDate()} ${TabStashTime.timeOfDayLabel(date)}`;
}
...
function normalizeAutoTitles(collections) {
  ...
  const next = collections.map((col) => {
    ...
    if (col.autoTitleType === 'timeOfDay') {
      if (col.name !== baseName) {
        changed = true;
        return { ...col, name: baseName, createdAt };
      }
      return col;
    }

    const legacyMatch = LEGACY_TIMESTAMP_REGEX.exec(col.name || '');
    const weekdayMatch = LEGACY_WEEKDAY_REGEX.exec(col.name || '');
    if (!legacyMatch && !weekdayMatch) return col;
