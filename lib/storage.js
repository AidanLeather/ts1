...
const Storage = {
  ...
  async moveTab(tabId, targetCollectionId) {
    ...
    if (source) source.last_modified = Date.now();
  ...
  async updateTab(tabId, updates) {
    ...
    for (const col of collections) {
      ...
      if (!tab.archived && oldUrl && oldUrl !== newUrl && urlIndex[oldUrl]) {
        urlIndex[oldUrl]--;
      ...
      if (!tab.archived && newUrl && oldUrl !== newUrl) {
  ...
  async toggleTabStar(tabId) {
    ...
    for (const col of collections) {
      ...
      if (tab) {
        ...
        col.last_modified = Date.now();
        await this.saveCollections(collections);
        await this.logAction('star', { tabId, collectionId: col.id, starred: tab.starred });
        return tab.starred;
      }
    }
    return false;
  ...
  async mergeDuplicates(url, targetCollectionId) {
    ...
    for (const inst of instances) {
      ...
      inst.col.last_modified = Date.now();
    }
    ...
    target.tabs.unshift({
      ...
      archived: false,
      starred: instances.some((i) => i.tab.starred),
      tags: allTags,
    });
    target.last_modified = Date.now();
  ...
  async mergeAllDuplicates() {
    ...
    for (const [url, instances] of Object.entries(urlMap)) {
      ...
      const allTags = [...new Set(instances.flatMap((i) => i.tab.tags || []))];
      const anyStarred = instances.some((i) => i.tab.starred);
      ...
      targetCol.last_modified = Date.now();
  ...
  sortCollections(collections) {
    return [...collections].sort((a, b) => {
      ...
      if (!a.isPinned && b.isPinned) return 1;
      const aTime = a.last_modified ?? a.lastModified ?? a.createdAt ?? 0;
      const bTime = b.last_modified ?? b.lastModified ?? b.createdAt ?? 0;
      return bTime - aTime;
    });
  },
};
