...
function normalizeAutoTitles(collections) {
  ...
  const next = collections.map((col) => {
    ...
    if (Array.isArray(normalizedCol.tabs)) {
      const nextTabs = normalizedCol.tabs.map((tab) => {
        ...
        if (tab.archived === undefined) {
          changed = true;
          normalizedTab = { ...normalizedTab, archived: false };
        }
        if (normalizedTab.lastAccessedAt === undefined) {
          changed = true;
          normalizedTab = { ...normalizedTab, lastAccessedAt: null };
        }
    ...
    if (normalizedCol.lastAccessedAt === undefined) {
      changed = true;
      normalizedCol = { ...normalizedCol, lastAccessedAt: null };
    }
...
const Storage = {
  ...
  async touchCollectionAccessed(collectionId) {
    const collections = await this.getCollections();
    const col = collections.find((c) => c.id === collectionId);
    if (!col) return;
    col.lastAccessedAt = Date.now();
