...
const DEFAULT_SETTINGS = {
  ...
  keepDurationDays: 7,
  dailyAutomaticBackup: true,
};

const BACKUP_INSTANCE_ID_KEY = 'backupInstanceId';
const FRIENDLY_ADJECTIVES = ['amber', 'brisk', 'calm', 'clever', 'lucky', 'mellow', 'quiet', 'solar', 'swift', 'tidy'];
const FRIENDLY_NOUNS = ['badger', 'falcon', 'harbor', 'lantern', 'maple', 'meadow', 'otter', 'ridge', 'river', 'spruce'];

function randomItem(arr) {
  if (!Array.isArray(arr) || arr.length === 0) return 'whytab';
...
function createFriendlyBackupInstanceId() {
  return `${randomItem(FRIENDLY_ADJECTIVES)}-${randomItem(FRIENDLY_NOUNS)}-${randomSuffix()}`;
}
...
function normalizeSettings(settings = {}) {
  ...
  return {
    ...merged,
    autoArchiveAfterDays: clampNumber(merged.autoArchiveAfterDays, 1, 90, DEFAULT_SETTINGS.autoArchiveAfterDays),
    keepDurationDays: clampNumber(merged.keepDurationDays, 1, 30, DEFAULT_SETTINGS.keepDurationDays),
  };
}
...
function normalizeCollectionMetadata(collections) {
  ...
  const next = collections.map((col) => {
    ...
    if (normalizedCol.keptAt === undefined) {
      normalizedCol = { ...normalizedCol, keptAt: null };
      changed = true;
    }
...
function logActivityEvent(type, metadata = {}) {
  try {
    if (globalThis.WhyTabActivity?.logActivity) {
      WhyTabActivity.logActivity(type, metadata);
    }
  } catch (_err) {
...
const Storage = {
  ...
  async getOrCreateBackupInstanceId() {
    const stored = await chrome.storage.local.get(BACKUP_INSTANCE_ID_KEY);
    const existing = String(stored?.[BACKUP_INSTANCE_ID_KEY] || '').trim();
    if (existing) return existing;

    const generated = createFriendlyBackupInstanceId();
    await chrome.storage.local.set({ [BACKUP_INSTANCE_ID_KEY]: generated });
    return generated;
  },
  ...
  async addCollection(name, tabs, options = {}) {
    ...
    logActivityEvent('collection_created', {
      collectionId: collection.id,
      isUserNamed: Boolean(collection.isUserNamed),
    });
  ...
  async removeTab(tabId) {
    ...
    for (const col of collections) {
      ...
      if (idx !== -1) {
        ...
        logActivityEvent('tab_deleted', { tabId, collectionId: col.id });
        break;
      }
    }
  ...
  async moveTab(tabId, targetCollectionId, targetIndex = null, options = {}) {
    const { touchCollections = true } = options;
    const collections = await this.getCollections();
    let tab = null;
    ...
    if (sourceCollection && touchCollections) {
  ...
  async removeCollection(collectionId) {
    ...
    if (col) {
      ...
      logActivityEvent('collection_deleted', { collectionId, tabCount: (col.tabs || []).length });
    }
  ...
  async setCollectionArchived(collectionId, archived) {
    ...
    if (archived) {
      logActivityEvent('collection_archived', { collectionId, tabCount: (col.tabs || []).length });
    }
    return col;
  },
  ...
  async setTabArchived(tabId, archived) {
    ...
    for (const col of collections) {
      ...
      await this.saveCollections(collections);
      if (archived) {
        logActivityEvent('tab_archived', { tabId, collectionId: col.id });
      }
      return tab;
    }
    return null;
  ...
  async renameCollection(collectionId, newName) {
    ...
    if (col) {
      ...
      await this.saveCollections(collections);
      logActivityEvent('collection_renamed', { collectionId, newName });
    }
  },
  ...
  async setCollectionKeptAt(collectionId, keptAt) {
    const collections = await this.getCollections();
    const col = collections.find((c) => c.id === collectionId);
    if (!col) return null;
    col.keptAt = keptAt == null ? null : Number(keptAt);
    await this.saveCollections(collections);
    return col.keptAt;
  },
  ...
  async togglePin(collectionId) {
    ...
    if (col) {
      ...
      await this.saveCollections(collections);
      if (col.isPinned) {
        logActivityEvent('collection_pinned', { collectionId });
      }
      return col.isPinned;
    }
    return false;
    ...
    for (const col of collections) {
      ...
      if (tab) {
        ...
        if (normalized && !tab.tags.includes(normalized)) {
          ...
          logActivityEvent('tag_added', { tabId, tag: normalized });
        }
        return tab.tags;
      }
    }
  ...
  search(collections, query, filters = {}) {
    ...
    for (const col of collections) {
      ...
      if (matchedTabs.length || collectionMatch) {
        results.push({
          collectionId: col.id,
          collectionName: col.name,
          collectionArchived: Boolean(col.archived),
          collectionNameMatch: collectionMatch,
          isPinned: Boolean(col.isPinned),
          isUserNamed: Boolean(col.isUserNamed),
          createdAt: col.createdAt || 0,
          totalTabs: (col.tabs || []).length,
          matchedTabs,
        });
      }
    }
